...
The loader keeps header and pattern data in Fast/Slow RAM when available, and sample data in Chip RAM for Paula DMA. Autoplay and Loop use the embedded ptplayer song-end flag and do not depend on fixed playback timeouts. If neither mode is enabled, playback stops when the song reaches its end.
