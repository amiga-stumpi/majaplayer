...
static void check_playback_end(struct AppState *app)
{
    if (!g_player_active || mt_SongEnd == 0)
        return;
    if (app->loop) {
        mt_SongEnd = 0;
        return;
...
static void toggle_loop(struct AppState *app)
{
    app->loop = !app->loop;
    if (app->loop) {
        app->autoplay = 0;
        mt_SongEnd = 0;
        set_status(app, "Loop on");
    } else {
        set_status(app, "Loop off");
    }
    redraw(app);
}
...
static void draw_info_window(struct Window *win)
{
    ...
    SetDrMd(rp, JAM1);
    Move(rp, 12, 24);
    Text(rp, (STRPTR)"MajaRadio for Kick1.3", str_len("MajaRadio for Kick1.3"));
    Move(rp, 12, 38);
    Text(rp, (STRPTR)"Version: ", str_len("Version: "));
    Text(rp, (STRPTR)MAJA_INFO_VERSION, str_len(MAJA_INFO_VERSION));
    Move(rp, 12, 52);
    Text(rp, (STRPTR)"by Marcel Jaehne", str_len("by Marcel Jaehne"));
    Move(rp, 12, 66);
    Text(rp, (STRPTR)"(c) 2026", str_len("(c) 2026"));
    Move(rp, 12, 84);
    Text(rp, (STRPTR)"If you want to buy me a coffe,", str_len("If you want to buy me a coffe,"));
    Move(rp, 12, 98);
    Text(rp, (STRPTR)"send me a buck to:", str_len("send me a buck to:"));
    Move(rp, 12, 112);
    Text(rp, (STRPTR)"https://paypal.me/mytubefree", str_len("https://paypal.me/mytubefree"));
    ok.x = 129;
    ok.y = 132;
    ok.w = 42;
    ok.h = 14;
